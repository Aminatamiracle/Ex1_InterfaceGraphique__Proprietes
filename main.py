...
# Importer la classeListViewDialog
from ClasseListViewDialog import *
...
class fenetrePrincipale(QtWidgets.QMainWindow, interfacegraphique.Ui_MainWindow):
    ...
    def on_pushButton_supprimer_clicked(self):
        ...
        if etud.NomEtud != "" and etud.NumEtud != "" and etud.DateNaiss != "" and verifier_etudiant is True:
            ...
            if not trouve:
                ...
                self.lineEdit_nom.clear()
                self.dateEdit_DNaiss.setDate(QDate(2000, 1, 1))
    # le gestiomnaire d'événement du bouton Afficher listview 
    @pyqtSlot()
    def on_pushButton_afficher_listview_clicked(self):
        ...
        # Associer le modèle à ma listeView pour pouvoir faire le append
        dialog.listesViewEtudiants.setModel(model)
        for elt in ls_Etudiants :
            item = QStandardItem(elt.NumEtud + " * " + elt.NomEtud + " * " + elt.Programme)
            model.appendRow(item)
        # Afficher la boie de  dialogue
        dialog.setModal(True)
        #Afficher la fenetre listeView
        dialog.show()
        # Executer la boite de dialog
        dialog.exec_()
